...
export const ENV_VARS: EnvVarSpec[] = [
  ...
  { name: "MAGICFLUX_WEBHOOK_URL", category: "optional-integration", description: "MagicFlux AI Lead Qualification — the specific deployed workflow's webhook URL (https://<host>/api/workflows/<workflowId>/webhook). Missing → the project-inquiry form still saves the lead normally; MagicFlux forwarding is silently skipped." },
  { name: "MAGICFLUX_WEBHOOK_SECRET", category: "optional-integration", description: "That workflow's own per-workflow webhook secret (from MagicFlux's dashboard, GET /api/workflows/[id]/webhook-secret), sent as X-MagicFlux-Webhook-Secret. Never logged." },
